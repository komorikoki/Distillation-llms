...
def make_data(data):
    ...
    for text in tqdm(dataset, desc="Tokenizing dataset"):
        [cq, ans] = devide(text)
        tokenized = tokenizer(cq, padding="max_length", max_length=512, truncation=True, return_tensors="pt")
...
for model_name in directories:
    ...
    for i in tqdm(range(size_v)):
        ...
        for j in range(4):
            acc += accuracy(output_ids[j], ans[i*4+j], ignore_len=512)
    
    print(f"{model_name}acc: {(acc/size_v):.3f}")
    
    model_names.append(model_name)
    accs.append(acc)

for i in range(len(model_names)):
    print(f"{model_names[i]}: acc {(accs[i]/size_v):.3f}")
