...
for j in range(5):
    for i in tqdm(range(size)):
        ...
        teacher_probs = torch.softmax(teacher_logits / temperature, dim=-1)  # 温度（temperature）を使用
        probs = torch.log_softmax(logits / temperature, dim=-1)

        kl_loss = kl_loss_fn(probs, teacher_probs)
        cr_loss = criterion(logits.view(-1, vocab_size), labels.view(-1))
        loss = alpha * kl_loss + (1-alpha) * cr_loss
...
plt.savefig('eval_loss_plot12.png')

student_model.save_pretrained("./finetuned_model5")
