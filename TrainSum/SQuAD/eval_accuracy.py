...
with open("output_acc3.txt", "w") as f:
    for i in tqdm(range(size_v)):
        ...
        for j in range(4):
            ...
            f.write(str(4*i+j) + " label:" + tokenizer.decode(input_ids_v[j][250:255]) +"\n")
            f.write(str(4*i+j) + " ans:" + tokenizer.decode(ans[i*4+j][1:]) +"\n")
            f.write(str(4*i+j) + " model:" + tokenizer.decode(output_ids[j][256:256+seq_len+10]) +"\n")
